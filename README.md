...
本仓库不会内置真实 PDF、EPUB、DOCX 或其他书籍样本。请自行准备你合法拥有、允许本地处理的测试文件，并放入 `raw/books/`。`raw/books/` 已被 `.gitignore` 忽略，不应提交到仓库。
...
如果缺少转换工具，请先运行 `check_tools.py` 查看当前环境。v0.1.1 不支持自动安装依赖。
...
- `check_tools.py` 增加 `--install` 参数，用于可选的依赖安装引导
- 自动生成章节笔记骨架
- 扫描版 PDF 的 OCR 流程编排
- 断点续跑和 `progress.json`
